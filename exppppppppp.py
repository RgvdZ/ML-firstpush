...
def CreateData(N : int  , M : int , inp : str , out : str  ) -> pd.DataFrame :   #Define a function CreateData to create synthetic data. The type of data (discrete or real) for both input and output can be specified.
    """
...
def TimingTrees(N : int, M : int , T : int):   #Define a function TimingTrees to measure the time taken by the fit and predict methods of a Decision Tree for different types of data. It creates four different types of data (discrete input and output, discrete input and real output, real input and discrete output, real input and output), trains a Decision Tree on them, and measures the time taken. The results are stored in numpy arrays.
    """
...
def PlotTimeChart(data : np.array , label ):   #Define a function PlotTimeChart to plot the timing results. It creates a 3D plot with the number of samples and features on the x and y axes, and the time taken on the z axis.
...
if __name__ == '__main__':
    TimingTrees(25,25,10)    #If the script is run as a main program (not imported as a module), it calls the TimingTrees function with specific parameters.
